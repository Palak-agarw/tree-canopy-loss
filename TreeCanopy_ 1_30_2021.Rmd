...
TreeCanopyLoss <- 
  fishnet%>%
  mutate(GridArea = st_area(fishnet))%>%
  mutate(GridArea= as.numeric(GridArea))%>%
  st_join(TreeCanopyAll)%>%
  filter(CLASS_NAME == "Loss")%>%
  group_by(uniqueID, GridArea)%>%
  summarise(AreaLoss = sum(TreeArea))%>%
  mutate(pctLoss = AreaLoss / GridArea)
...
TreeCanopyCoverage<- 
  ...
  mutate(pctLoss = AreaCoverage / GridArea)
...
ggplot()+ 
  geom_sf(data = TreeCanopyCoverage, aes(fill = pctLoss))+ 
  scale_fill_viridis()+
