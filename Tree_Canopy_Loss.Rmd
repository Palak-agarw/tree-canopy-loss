...
TreeCanopyArea_census <- 
  treecanopy_census%>%
  group_by(tractfips,tractarea)%>%
  mutate(Area = sum(TreeArea))%>%
  st_drop_geometry()

phillyhealth <-
  merge(x = TreeCanopyArea_census, y = phillyhealth, by = "tractfips", all = TRUE)
...
phillyhealth <- phillyhealth %>%
  dplyr::select(tractfips,casthma_crudeprev,AreaLoss.x,pctGain,pctLoss,geometry,stroke_crudeprev,phlth_crudeprev,
                obesity_crudeprev,mhlth_crudeprev,lpa_crudeprev, TreeArea)%>%
