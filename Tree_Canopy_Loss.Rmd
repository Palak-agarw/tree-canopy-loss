...
treecanopy_census<- 
  ...
  mutate(TreeArea = as.numeric(TreeArea)) %>%
  select(tractfips, TreeArea) 

Treecanopy_census <-
  treecanopy_census %>%
  group_by(tractfips)%>%
  summarise(Area = sum(TreeArea))%>%
  st_drop_geometry()
...
phillyhealth <-
  merge(x = Treecanopy_census, y = phillyhealth, by = "tractfips", all = TRUE)
...
phillyhealth <- phillyhealth %>%
  dplyr::select(tractfips,casthma_crudeprev,,pctGain,pctLoss,geometry,stroke_crudeprev,phlth_crudeprev,
                obesity_crudeprev,mhlth_crudeprev,lpa_crudeprev, Area)%>%
...
correlation.long5 <-
  phillyhealth%>%
     dplyr::select(casthma_crudeprev, phlth_crudeprev,
                obesity_crudeprev,mhlth_crudeprev,lpa_crudeprev, Area) %>%
...
correlation.cor5 <-
  correlation.long5 %>%
    group_by(Variable) %>%
    summarize(correlation = cor(Value, Area, use = "complete.obs"))

ggplot(correlation.long5, aes(Value, Area)) +
  geom_point(size = 0.1) +
    scale_x_log10() + scale_y_log10() +
  geom_text(data = correlation.cor5, aes(label = paste("r =", round(correlation, 2))),
            x=-Inf, y=Inf, vjust = 1.5, hjust = -.1) +
  geom_smooth(method = "lm", se = FALSE, colour = "red") +
  facet_wrap(~Variable, ncol = 2, scales = "free") +
  labs(title = "Tree Canopy Coverage (2018)") +
...
```{r Imperviousness, fig.height=8, fig.width=8, message=FALSE, warning=FALSE, cache=TRUE, results='hide'}
...
poles_nn1 <- 
  TreeCanopy%>% 
  mutate(poles_nn = nn_function(st_c(st_centroid(.)), st_c(poles), 10))
...
TreeCanopyCentroids1<- 
  ...
  st_drop_geometry()%>%
  group_by(uniqueID)%>%
  summarise(avg_nnPole = mean(poles_nn))

FinalFishnet3<-
  TreeCanopyCentroids1%>% 
  left_join(FinalFishnet3, .)%>% 
  mutate(LogPole = log10(avg_nnPole))
  
#distMarineInletLinereg <-lm(LogPctLoss ~ LogPole, data = FinalFishnet3)
#summary(distMarineInletLinereg)

ggplot(FinalFishnet3, aes(y = LogPctLoss , x = LogPole))+
  geom_point()+
  labs(title = "Percent Change vs Construction nn1",
       subtilte = "Aggregated by Fishnet") +
  xlab("Average Percent Change ") +
...
FinalFishnet3$Variable <- ifelse((FinalFishnet3$pctChange > 0),1, 0)
...
# LOGISTIC MODEL
set.seed(1214)
trainIndex <- createDataPartition(FinalFishnet3,
                                  y = FinalFishnet3$
                                  p = .60, 
                                  list = FALSE)

finalTrain <- FinalFishnet3[ trainIndex,] %>% st_drop_geometry()
finalTest  <- FinalFishnet3[-trainIndex,] %>% st_drop_geometry()

inTrain <- caret::createDataPartition(
            y = FinalFishnet3$Variable,
            p = .60, list = FALSE)

# split data into training and test, before comma is row, after comma is column
Final.training <- FinalFishnet3[inTrain,] %>% st_drop_geometry()
Final.test     <- FinalFishnet3[-inTrain,] %>% st_drop_geometry()

# MODEL
finalModel <- glm(Final.training$Variable ~ .,
                    data=Final.training %>% 
                      dplyr::select(countConst, avg_nnConst,LogPole, LogAvgParcelSize,
                                    LogPole,LogPctGain,LogPctChange),
...
## Prediction
testProbs <- data.frame(Outcome = as.factor(Final.test$Variable),
                        Probs = predict(finalModel, Final.test, type= "response"))
