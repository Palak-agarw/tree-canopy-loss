...
### 1. How does tree canopy loss vary across neighborhoods and sociodemographic contexts?
```{r Tree Canopy 08, 18, message=FALSE, warning=FALSE, results='hide', fig.height = 6, fig.width = 9}

TreeSample <- sample_n(TreeCanopy, 1000)

ggplot() +
  geom_sf(data = TreeSample, colour = "Black", fill = "Green") +
  geom_sf(data = Neighborhood, colour = "Blue", fill = "Transparent") +
  labs(title = "XXXX") +
  mapTheme()
...
JuniataPark <- Neighborhood %>%  dplyr::filter(NAME=="JUNIATA_PARK")

JuniataPark <- st_intersection(st_make_valid(TreeCanopy), JuniataPark)


ggplot() +
  geom_sf(data = JuniataPark, aes(colour = CLASS_NAME)) +
  scale_fill_manual(values = paletteJ) +
  labs(title = "Tree Canopy Change in Juniata Park") +
  mapTheme()
```
