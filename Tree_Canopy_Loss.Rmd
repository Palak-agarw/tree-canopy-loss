...
```{r Tree Canopy Change, message=FALSE, warning=FALSE, cache=TRUE, include=TRUE, results='hide'}
...
```{r Where is Philadelphia achieving The 30 Percent Goal?, message=FALSE, warning=FALSE, results='hide', fig.height = 7, fig.width = 13, cache=TRUE}
...
TreeCanopyCoverageN18<- 
  Neighborhood%>%
  left_join(TreeCanopyCoverageN18)%>%
  mutate(Goal = ifelse(pctCoverage > 30, "Met Goal", "GoalNotMet"))
  

ggplot() + 
  geom_sf(data = TreeCanopyCoverageN18, aes(fill = q5(pctCoverage)))+ 
  scale_fill_manual(values = palette2,
                    labels = qBr(TreeCanopyCoverageN18, "pctCoverage"),
                    name = "Percent Tree Coverage, 2018\n(Quintile Breaks)")+
  labs(title= "Percent Tree Coverage by Neighborhood (2018)", 
       subtitle = "Tree Canopy Area / Neighborhood Size ")+
  mapTheme()
...
ggplot() +
  ...
  labs(title = "What Neighborhoods are losing trees?") +
  theme(plot.title = element_text(hjust = 0.5, size = 8))+
  mapTheme()

ggplot() +
  geom_sf(data = TreeCanopyCoverageN18, aes(fill = Goal)) +
  scale_fill_manual(values = palette3, name = "Reached 30%?")+ 
  labs(title = "Met 30% Goal?") +
  theme(plot.title = element_text(hjust = 0.5, size = 8))+
  mapTheme()
...
```{r Fishnet Aggregation, message=FALSE, warning=FALSE, results='hide', fig.height = 18, fig.width = 9}

grid.arrange(ncol=1,
             
ggplot()+ 
  geom_sf(data = FinalFishnet, aes(fill = q5(AreaGain)))+ 
  scale_fill_manual(values = palette2,
                    labels = qBr(FinalFishnet, "AreaGain"),
                    name = "Area Gain (ft^2)")+
  labs(title= "How Much Tree Canopy was Gained from 2008 - 2018?", 
       subtitle = "Philadelphia, Pa")+
  theme(plot.title = element_text(hjust = 0.3)) + mapTheme(),

ggplot()+ 
  geom_sf(data = FinalFishnet, aes(fill = q5(AreaLoss)))+ 
  scale_fill_manual(values = palette2,
                    labels = qBr(FinalFishnet, "AreaLoss"),
                    name = "Tree Canopy Loss (f^2) \n(Quintile Breaks)")+
  labs(title= "How Much Tree Canopy Was Lost from 2008 - 2018?")+
  theme(plot.title = element_text(hjust = 0.3)) + mapTheme(),

ggplot()+ 
  geom_sf(data = FinalFishnet, aes(fill = q5(AreaCoverage)))+ 
  scale_fill_manual(values = palette2,
                    labels = qBr(FinalFishnet, "AreaCoverage"),
                    name = "Total Tree Canopy \n(Quintile Breaks)")+
  labs(title= "Total Tree Canopy by Fishnet", 
       subtitle = "2018")+
  theme(plot.title = element_text(hjust = 0.3)) + mapTheme()) 
...
ggplot()+ 
  ...
  scale_fill_manual(values = palette2,
                    ...
                    name = "Percent Gain")+
  labs(title= "What was the Percent Tree Canopy Gain in Each Fishnet Cell?", 
...
ggplot()+ 
  ...
  scale_fill_manual(values = palette2,
                    ...
                    name = "Percent Loss")+
  labs(title= "What was the Percent Tree Canopy Loss in Each Fishnet Cell?", 
       subtitle = " Tree Canopy Loss / Total Tree Canopy Coverage (2008, 2018)")+
  theme(plot.title = element_text(hjust = 0.3)) + mapTheme(),
...
ggplot()+ 
  ...
  labs(title= " What is the Net Percent Gain or Loss from 2008- 2018?", 
       subtitle = "Percent Tree Canopy Gain (2008 - 2018) Minus Percent Tree Canopy Loss") +
  theme(plot.title = element_text(hjust = 0.3)) + mapTheme()) 
...
TreeCanopyCoverage18 <- 
  ...
  mutate(pctCoverage18 = AreaCoverage18 / 4635409)%>%
  dplyr::select(pctCoverage18, AreaCoverage18, uniqueID)%>%
  st_drop_geometry()
...
ggplot(FinalFishnet, aes(x = GainMinusLoss, y = AreaCoverage18))+
  geom_point()+ 
  labs(title = "Percent Tree Canopy Gain (2008 - 2018) Minust Percent Tree Canopy Loss \n vs. Percent Tree Canopy Coverage (2018)", 
       subtilte = "Aggregated by Fishnet") + 
