...
phillyhealth <- phillyhealth %>%
  dplyr::select(tractfips,casthma_crudeprev,AreaLoss.x,pctGain,pctLoss,geometry)%>%
...
correlation.long4 <-
  phillyhealth%>%
     dplyr::select(casthma_crudeprev, pctLoss) %>%
    gather(Variable, Value, -pctLoss) %>%
...
TreeCanopyCentroids2 <- 
  TreeCanopyCentroids%>% 
  filter(pctChange < 100)

ggplot(TreeCanopyCentroids2, aes(x = pctLoss , y = avg_nnConst))+
  geom_point()+ 
  labs(title = "Percent Change vs Construction nn1", 
       subtilte = "Aggregated by Fishnet") + 
  xlab("Percent Change ") + 
  ylab("Construction nn1") + 
  theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold"), plot.subtitle = element_text(hjust = 0.5, size = 8)) +
  plotTheme()
