...
library(ggplot2)
#Not working yet
JuniataPark <- st_intersection(Neighborhood %>%  dplyr::filter(NAME=="JUNIATA_PARK"), st_make_valid(TreeCanopy)) %>% dplyr::select(NAME, CLASS_NAME)
                                 

ggplot() +
  geom_sf(data = JuniataPark, aes(colour = CLASS_NAME, fill = CLASS_NAME)) %>%
