...
FinalFishnet <- 
  ...
  dplyr::select(pctLoss, pctGain, pctChange,
                AreaGain, AreaLoss, AreaCoverage08, AreaCoverage18, pctCoverage08, pctCoverage18, uniqueID, pctChange)
...
ggplot(FinalFishnet, aes(x = pctChange, y = AreaCoverage18))+
  geom_point()+ 
...
```{r Const_Permit, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height =  8, fig.width = 8 cache=TRUE}
...
LandUse <- st_read("C:/Users/Kyle McCarthy/Documents/Practicum/Data/LandUse/LandUses.csv")%>%
  mutate(AreaGained = as.numeric(AreaGained),
  ...
  mutate(pctLoss = as.numeric(AreaLoss / Coverage08), 
         pctGain = as.numeric(AreaGained/ Coverage18), 
         pctChange = (Coverage18 - Coverage08)/ (Coverage08) * 100,
         netChange = AreaGained - AreaLoss) 

LandUseLong<- 
  LandUse%>%
  mutate(AreaLoss = AreaLoss * -1)%>%
  dplyr::select(AreaGained, AreaLoss, Descriptio)%>%
  gather(variable, value, -c(Descriptio))%>% 
  mutate(variable = ifelse(variable == "AreaGained", "Area Gain", "Area Loss"))
...
ggplot(LandUseLong, aes(fill = variable, y=Descriptio, x=value))+
  ...
  labs(title = "Total Area Lossed or Gained by Land Use Type")+ 
...
ggplot(LandUseLong, aes(fill = variable, y=Descriptio, x=value))+
  ...
  scale_fill_manual(values = landusepal, 
                    name = "Area Gain or Loss")+
  labs(title = "Percent Tree Area Lost or Gained by Land Use Type")+ 
  xlab("Percent Area Lossed                          Percent Area Gained")+ 
  ylab("Land Use Type")
  
```

```{r Const_Permit, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height = 10, fig.width = 10, cache=TRUE}
...
ggplot(LandUse, aes(y=Descriptio, x=pctChange))+
  geom_bar(stat='identity', fill="forest green")+ 
  labs(title = "Percent Change Change", 
       subtitle = "'18 Tree Canopy Coverage - '08 Tree Canopy Coverage / \n '18 Tree Canopy Coverage * 100 ")+ 
  xlab("Land Use Type")+ 
  ylab("Total Net Change")+ 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))) 
