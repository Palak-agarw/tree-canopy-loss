...
phillyhealth <- phillyhealth %>%
  dplyr::select(tractfips,casthma_crudeprev.x,,pctGain.y,pctLoss.y,geometry,stroke_crudeprev.x,phlth_crudeprev.x,
                obesity_crudeprev.x,mhlth_crudeprev.x,lpa_crudeprev.x, TreeArea)%>%
  mutate(LogPctLoss = log10(pctLoss.y),
         LogPctGain = log10(pctGain.y))


correlation.long4 <-
  phillyhealth%>%
     dplyr::select(casthma_crudeprev.x, phlth_crudeprev.x,stroke_crudeprev.x,
                obesity_crudeprev.x,mhlth_crudeprev.x,lpa_crudeprev.x, pctLoss.y) %>%
    gather(Variable, Value, -pctLoss.y) %>%
  mutate(Value = as.numeric(Value))

correlation.cor4 <-
  correlation.long4 %>%
    group_by(Variable) %>%
    summarize(correlation = cor(Value, pctLoss.y, use = "complete.obs"))

ggplot(correlation.long4, aes(Value, pctLoss.y)) +
  geom_point(size = 0.1) +
    scale_x_log10() + scale_y_log10() +
...
correlation.long5 <-
  phillyhealth%>%
     dplyr::select(casthma_crudeprev.x, phlth_crudeprev.x,
                obesity_crudeprev.x,mhlth_crudeprev.x,lpa_crudeprev.x, TreeArea) %>%
    gather(Variable, Value, -TreeArea) %>%
  mutate(Value = as.numeric(Value))

correlation.cor5 <-
  correlation.long5 %>%
    group_by(Variable) %>%
    summarize(correlation = cor(Value, TreeArea, use = "complete.obs"))

ggplot(correlation.long5, aes(Value, TreeArea)) +
  geom_point(size = 0.1) +
    scale_x_log10() + scale_y_log10() +
