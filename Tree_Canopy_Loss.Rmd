...
FinalFishnet %>%
  select(pctLoss, holc_grade) %>%
  group_by(holc_grade) %>%
  summarize(mean_loss = mean(pctLoss)) %>%
  ggplot()+
  geom_histogram(aes(mean_loss), binwidth = 1, fill = "red")+
...
FinalFishnet5 <-
  ...
  dplyr::select(pctCoverage18, LogAvgParcelSize,Loge311Count, LogPctRes, avg_nnConst,avg_nnPole,
                                    ...
                                    pctTrans, LogPctResTrans, population, italian, medHHInc, Variable, pctCoverage08Cat, LogPctLoss) #, holc_grade
...
## Prediction
testProbs2 <- data.frame(Outcome = finalTest$LogPctLoss,
