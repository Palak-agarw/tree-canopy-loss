...
ggplot() +
  geom_sf(data = FishnetIntersect, fill = "light blue", colour = "white") +
...
```{r 2018 Tree Canopy, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE, results='hide'}
...
```{r Net Change,  message=FALSE, warning=FALSE, results='hide', fig.height = 8, fig.width = 8, cache=TRUE}
b <- ggplot()+ 
...
```{r Neighborhood level Tree Loss, message=FALSE, warning=FALSE, results='hide', fig.height = 9, fig.width = 13, cache = TRUE}
library(magrittr)
...
```{r Demographics, message=FALSE, warning=FALSE, results='hide', fig.height = 18, fig.width = 13,  cache=TRUE}
...
```{r Hydrology and Loss, fig.height = 12,fig.width = 12, message=FALSE, warning=FALSE, include=TRUE, results='hide', cache=TRUE}
...
```{r HOLC categories, message=FALSE, warning=FALSE, results='hide', cache=TRUE}
...
```{r Construction_scenarios, message=FALSE, warning=FALSE, fig.height = 6, fig.width = 6, cache=TRUE}

FinalFishnet5$countConst1 <- FinalFishnet5$countConst * 0.75
FinalFishnet5$countConst2 <- FinalFishnet5$countConst * 0.50
FinalFishnet5$countConst3 <- FinalFishnet5$countConst * 2
...
ggplot() +
  ...
  scale_fill_manual(values=palette5, name="Risk Score") +
  labs(title="Predicted Risk Scores Across Philadelphia given \nCounstruction counts increased by 200%")+
  mapTheme()
...
``````{r CV, message=FALSE, warning=FALSE, fig.height = 6, fig.width = 6, cache=TRUE}  
...
ggplot() +
  ...
  geom_sf(data = FinalFishnet5, fill = "transparent", color = "black", size=.5)+
