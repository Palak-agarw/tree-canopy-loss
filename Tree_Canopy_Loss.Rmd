...
FinalFishnet$pctLossCat <- cut(FinalFishnet$pctLoss, 
                       ...
                       right = FALSE)
```
```{r Existing Canopy fishnet, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE, results='hide'}
ggmap(base_map) +
...
```{r Net Canopy change,  message=FALSE, warning=FALSE, results='hide', fig.height = 6, fig.width = 6, cache=TRUE}
...
```{r ACS tracts, message=FALSE, warning=FALSE, results='hide', fig.height = 13, fig.width = 13, cache = TRUE}
ACS_select <- ACS %>%
...
```{r HOLC rating Communities, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height =  7, fig.width = 7, cache='TRUE'}
...
```{r HOLC plot, fig.height=6, fig.width=12, message=FALSE, warning=FALSE, cache=TRUE, results='hide'}
# library(grid)
...
```{r Residential and transportation, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height = 4, fig.width = 10, cache=TRUE}
...
```{r residential density, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height = 3, fig.width = 8, cache=TRUE}
resDensity
```
...
The random forest predicts the probability of tree canopy loss occurring in a given grid cell from 0 to 1.To run our model we randomly split the data into a test set and a training set. Our training set is the data we "trained" our model on. Based on the training set, our model predicted if substantial tree canopy loss occurred in each grid within the test set. There are four potential outcomes to the model: 

1. True Positive - Indicates that the model correctly predicted that a tree canopy loss would occur in a given grid cell at a given threshold. 

2. True negative - Our model correctly predicted that a tree canopy loss would not occur. 

3. False Positive - The model predicted tree canopy loss, but no substantial tree canopy loss occurred.

4. False Negative- Indicates that the model predicted no substantial tree canopy loss, but there was tree canopy loss.

To validate our model, we chose a probability threshold above which tree canopy loss is most substantial. By looking at the threshold with the probabilities, we chose 25% percent tree canopy loss to be our threshold as that maximized true positives while reducing false negatives. The graph above visualizes confusion metrics for every possible threshold from 0 to 1. At .25, the true positive rate is high, and the false-negative rate is relatively low. A lower point would increase the true positive rate and decrease the false-negative rate, but the false positive rate would also increase. This could lead to distrust in the application's predictions, which may reduce compliance.
...
```{r roc curve, message=FALSE, warning=FALSE, results='hide', fig.height =  8, fig.width =8,cache=TRUE}
# ROC Curve
## This us a goodness of fit measure, 1 would be a perfect fit, .5 is a coin toss
