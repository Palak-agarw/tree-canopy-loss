...
fitControl <- trainControl(method = "cv", number = 100, classProbs=TRUE, summaryFunction=twoClassSummary)
set.seed(825)

reg.cv <- 
  train(FinalFishnet5$Variable ~ ., data = st_drop_geometry(FinalFishnet5) %>% 
                                dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, holc_grade, HydrologyPct), 
     method = "glm", trControl = fitControl, na.action = na.pass)
