...
## Adding HOLC grade to Fishnet
FinalFishnet4 <- 
  HOLC %>%
  st_make_valid() %>%
  st_intersection(FinalFishnet)%>%
  select(uniqueID, holc_grade)%>%
...
FinalFishnet5$Variable <- ifelse((FinalFishnet5$pctChange > 0),1, 0)
