...
FinalFishnet5$Variable <- ifelse(FinalFishnet5$pctLoss > 25,0, 1)
...
## Prediction
testProbs_const1 <- data.frame(Outcome = as.factor(finalTest$Variable),
                        Probs = predict(rf1, finalTest, type= "response"))
...
testProbs_const1$uniqueID <- seq.int(nrow(finalTest))

construction1 <- left_join(finalTest2, testProbs_const1)
...
## Prediction
testProbs_const2 <- data.frame(Outcome = as.factor(finalTest$Variable),
                        Probs = predict(rf2, finalTest, type= "response"))
...
testProbs_const2$uniqueID <- seq.int(nrow(finalTest))

construction2 <- left_join(finalTest2, testProbs_const2)
...
## Prediction
testProbs_const3 <- data.frame(Outcome = as.factor(finalTest$Variable),
                        Probs = predict(rf3, finalTest, type= "response"))
...
testProbs_const3$uniqueID <- seq.int(nrow(finalTest))

construction3 <- left_join(finalTest2, testProbs_const3)
...
## Prediction
testProbs_const4 <- data.frame(Outcome = as.factor(finalTest$Variable),
                        Probs = predict(rf4, finalTest, type= "response"))
...
testProbs_const4$uniqueID <- seq.int(nrow(finalTest))

construction4 <- left_join(finalTest2, testProbs_const4)
...
st_write(construction4, "construction-50plus.geojson")
st_write(construction3, "construction-25plus.geojson")
st_write(construction2, "construction-50less.geojson")
st_write(construction1, "construction-25less.geojson")
...
FinalFishnet5$Variable2 <- ifelse(FinalFishnet5$Variable == 0, "yes", "no")

FinalFishnet5 %>% 
  mutate(Variable = factor(Variable, 
                        labels = make.names(levels(Variable))))

reg.cv <-train(FinalFishnet5$Variable ~ ., data = st_drop_geometry(FinalFishnet5) %>% 
                                dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, Loge311Count, pctTrans, LogPctRes, holc_grade, HydrologyPct),
               method="rpart", family="binomial", trControl = ctrl)
