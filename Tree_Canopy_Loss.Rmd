...
# hydro + scale_fill_discrete(palette = "YlGnBu", direction = -1, aesthetics = c("colour", "fill"), guide = guide_legend(reverse = TRUE))
```
...
HOLC2 <- HOLC %>%
  st_intersection(st_make_valid(fishnet), HOLC) %>%  ##replaced Philadelphia with fishnet
  st_union() %>%
  st_as_sf()

HOLC2 <- HOLC2 %>%
  st_sym_difference(fishnet, HOLC2) %>%
  st_as_sf() %>%
