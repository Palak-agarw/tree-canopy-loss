...
rf <- randomForest(
  ...
  data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, Loge311Count, pctRes,
                                    pctTrans, LogPctRes, holc_grade, HydrologyPct)
)
...
reg.cv <-train(FinalFishnet5$Variable ~ ., data = st_drop_geometry(FinalFishnet5) %>% 
                                dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, Loge311Count, pctRes, pctTrans, LogPctRes, holc_grade, HydrologyPct) %>%
                                dplyr::mutate(Variable = ifelse(FinalFishnet5$Variable == 0,"X0","X1")),
