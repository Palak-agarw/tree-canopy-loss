...
FinalFishnet5$pctLoss <- ifelse(is.infinite(FinalFishnet5$pctLoss),0, FinalFishnet5$pctLoss)
...
FinalFishnet5 <-
  ...
  dplyr::select(pctCoverage18, LogAvgParcelSize,Loge311Count, LogPctRes, avg_nnConst,avg_nnPole,
                                    ...
                                    pctTrans, LogPctResTrans, population, italian, medHHInc, Variable, pctCoverage08Cat, pctLoss) #, holc_grade
...
# MODEL
finalModel <- glm(finalTrain$pctLoss ~ .,
                    data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, HydrologyPct),#, LogPctResTrans, population, e311Count, pctTransRes,  holc_grade
...
rf <- randomForest(
  ...
  data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, HydrologyPct),
  na.action = na.roughfix#, holc_grade
)
...
## Prediction
testProbs2 <- data.frame(Outcome = as.factor(finalTest$pctLoss),
...
cvFitkitchensink <-train(finalTrain$Variable ~ .,
  data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, HydrologyPct), #, holc_grade
