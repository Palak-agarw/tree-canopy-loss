...
left_join(FinalFishnet, .) %>%
mutateall(funs(replace_na(.,0)))%>%
mutate(HydrologyPct = HydrologyArea /2608225 * 100) 

ggplot() +
  geom_sf(data = HydrologyNet, colour = "blue") +
  mapTheme()
# 
# hydrology_net <-
#   Hydrology%>%
...
```{r Exploring Residential Further, message=FALSE, warning=FALSE, include=TRUE, results='hide', fig.height = 5, fig.width = 5, cache=TRUE}
...
finalModel <- glm(finalTrain$Variable ~ .,
                    data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    ...
                                    pctTrans,  italian, medHHInc, LogPctRes, holc_grade),#, LogPctResTrans, population, e311Count, pctTransRes
