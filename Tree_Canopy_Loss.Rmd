...
rf <- randomForest(
  ...
  data=finalTrain %>% 
                     dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,LogPctTrans, LogPole,
                                    countConst, Loge311Count, LogPctRes, holc_grade, HydrologyPct)
)
...
factor(FinalFishnet5$Variable)

factor(FinalFishnet5$Variable, levels = c(0,1), labels = c("zero", "one"))

FinalFishnet5$Variable2 <- ifelse(FinalFishnet5$Variable == 0, "zero", "one")
  
FinalFishnet5$Variable2 <- as.factor(FinalFishnet5$Variable)
