...
HOLC <- full_join(as.data.frame(HOLC), as.data.frame(HOLC2)) %>%
  st_as_sf()
...
FinalFishnet5 <-
  FinalFishnet5 %>%
  dplyr::select(pctCoverage18, LogAvgParcelSize,Loge311Count, LogPctRes, avg_nnConst,avg_nnPole, pctLoss, LogPctLoss,
...
``````{r Linear Model, message=FALSE, warning=FALSE, fig.height = 6, fig.width = 6}  

FinalFishnet5$LogPctLoss <- ifelse(is.na(FinalFishnet5$LogPctLoss),0, FinalFishnet5$LogPctLoss)

inTrain <- createDataPartition(
              y = FinalFishnet5$LogPctLoss, 
              p = .60, list = FALSE)
L.training <- FinalFishnet5[inTrain,] 
L.test <- FinalFishnet5[-inTrain,]  

reg.training <- lm(L.training$pctLoss ~ ., data = st_drop_geometry(L.training) %>% 
                                    dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, HydrologyPct, LogPctLoss))


Phil.test <-
  L.test %>%
  mutate(SalePrice.Predict = predict(reg.training, L.test),
         SalePrice.Error = SalePrice.Predict - LogPctLoss,
         SalePrice.AbsError = abs(SalePrice.Predict - LogPctLoss),
         SalePrice.APE = (abs(SalePrice.Predict - LogPctLoss)) / SalePrice.Predict)

mean(Phil.test$SalePrice.AbsError, na.rm = T)

mean(Phil.test$SalePrice.APE, na.rm = T)

fitControl <- trainControl(method = "cv", number = 400)
set.seed(825)

reg.cv <- 
  train(LogPctLoss ~ ., data = st_drop_geometry(FinalFishnet5) %>% 
                                dplyr::select(pctCoverage18, LogAvgParcelSize, avg_nnConst,avg_nnPole,
                                    countConst, pctWhite, pctBach, Loge311Count, pctRes,
                                    pctTrans,  italian, medHHInc, LogPctRes, HydrologyPct, LogPctLoss), 
