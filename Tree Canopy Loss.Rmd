...
FishnetFacet <-
  FinalFishnet %>%
  st_drop_geometry()%>%
    gather(Variable, Value,-AreaGain, -AreaLoss, -AreaCoverage, -uniqueID)%>%
  left_join(dplyr::select(fishnet, uniqueID))%>%
  dplyr::select(Variable, Value, AreaGain, AreaLoss, AreaCoverage, geometry)%>%
...
vars <- unique(FishnetFacet$Variable)
varList <- list()

for(i in vars){
  varList[[i]] <- 
    ggplot() +
      geom_sf(data = filter(FishnetFacet, Variable == i), 
...
```{r Additional Fishnet/Neighborhood Visualization, message=FALSE, warning=FALSE}
FinalNeighborhood <- 
  FinalNeighborhood %>%
  mutate(LossOrGain = ifelse(GainMinusLoss > 0, "Gain", "Loss"), 
...
```{r Additional Fishnet/Neighborhood Visualizations, message=FALSE, warning=FALSE, fig.height = 13, fig.width = 9}
