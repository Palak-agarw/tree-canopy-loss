...
FinalFishnet <- 
  ...
  mutate(GainToLossM = pctGain - pctLoss, 
         GainToLossD = pctGain/pctLoss * 100)%>%
  dplyr::select(pctLoss, pctGain, GainToLossM, GainToLossD,
                AreaGain, AreaLoss, AreaCoverage, uniqueID)
...
ggplot()+ 
  geom_sf(data = Philadelphia, fill = "#A9A9A9")+
  geom_sf(data = FinalFishnet, aes(fill = q5(GainToLossM)))+ 
  scale_fill_manual(values = palette2,
                    labels = qBr(FinalFishnet, "GainToLossM"),
...
ggplot(FinalFishnet, aes(x = pctLoss, y = AreaCoverage))+ 
  geom_point()+ 
  geom_smooth(method = 'gam', formula = y ~ poly(x, 2), se = FALSE)+
  labs(title = "AreaCoverage VS. Pct TreeCanopyLoss")

ggplot(FinalFishnet, aes(x = pctGain, y = AreaCoverage))+ 
  geom_point()+ 
  geom_smooth(method = 'gam', formula = y ~ poly(x, 2), se = FALSE)+
  labs(title = "AreaCoverage VS. Pct TreeCanopyGain")

ggplot(FinalFishnet, aes(x = GainToLossM, y = AreaCoverage))+ 
  geom_point()+ 
