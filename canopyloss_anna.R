...
Our use case mainly includes these 4 important questions and explorations:   
  1. What are the risk factors for tree canopy loss?  
  2. Which neighborhood do not meet the 30% tree canopy coverage?   
  3. How do current patterns of tree canopy coverage and loss reflect disinvestment as a result of redlining and older planning practices?  
  4. After identifying the neighborhoods, where and how should PPR or other agencies intervene?  
...
palette7 <- c("white","#c7ffd5","#81d4ac","#329D9C","#205072", "#0B0138")
...
1. How does tree canopy loss vary across neighborhoods and sociodemographic contexts?
  2. How is tree canopy loss affected by development?
  3. What’s the relationship between tree canopy health and built density in neighborhoods? (eg effect of urban heat island)
...
```{r Tree Canopy 08, 18, message=FALSE, warning=FALSE, results='hide', fig.height = 6, fig.width = 9, cache=TRUE}

library(ggplot2)

grid.arrange(ncol=2,
             ggplot()+ 
               geom_sf(data =   TreeCanopy%>%
                         filter(CLASS_NAME != "Gain"), fill = "black", color = NA)+ 
               labs(title= "Tree Canopy Coverage, 2008 ")+
               theme(plot.title = element_text(hjust = 0.1, size= 12))+ mapTheme(), 
             
             ggplot()+ 
               geom_sf(data =   TreeCanopy%>%
                         filter(CLASS_NAME != "Loss"), fill = "black", color = NA)+ 
               labs(title= "Tree Canopy Coverage, 2018 ")+
               theme(plot.title = element_text(hjust = 0.1, size= 12))+
               mapTheme())
```

```{r Tree Canopy Change, message=FALSE, warning=FALSE, results='hide', fig.height = 8, fig.width = 9, cache=TRUE}
paletteChange <- c("dark green", 'red', 'yellow')
ggplot()+ 
...
ggplot() +
  ...
  scale_fill_manual(values = palette7, name = "Gain or Loss")+ 
...
```{r Fishnet Aggregation2, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE, results='hide'}
...
```{r Fishnet Aggregation, message=FALSE, warning=FALSE, results='hide', fig.height = 8, fig.width = 15, cache=TRUE}
...
```{r Demographics, message=FALSE, warning=FALSE, include=TRUE, results='hide', cache=TRUE}
